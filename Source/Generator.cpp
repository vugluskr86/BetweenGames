...
namespace BWG {
   namespace Game {
      ...
      Direction GetRandomDirection(RandomGenerator& g) {
         return Direction(std::uniform_int_distribution<int>(0, 3)(g));
      }
      ...
      void DugneonGenerator::InitLayouts(TileMapType& map)
      {
         assert(self->_maxFeatures > 0 && self->_maxFeatures <= 100);
         assert(self->_xSize > 3 && self->_xSize <= 80);
         assert(self->_ySize > 3 && self->_ySize <= 25);
      ...
      bool DugneonGenerator::MakeCorridor(TileMapType& map, int x, int y, int maxLength, Direction direction)
      {
         assert(x >= 0 && x < self->_xSize);
         assert(y >= 0 && y < self->_ySize);

         assert(maxLength > 0 && maxLength <= std::max(self->_xSize, self->_ySize));

         //TileMapLayout& staticLayout = map.GetLayout(TileMap::eLayouts::TL_STATIC);

         auto length = Utils::GetRandomInt(2, maxLength, self->_random);
      ...
      bool DugneonGenerator::MakeStaticRoom(TileMapType& map, int x, int y, int xLength, int yLength, Direction direction)
      {
         //TileMapLayout& staticLayout = map.GetLayout(TileMap::eLayouts::TL_STATIC);
      ...
      bool DugneonGenerator::MakeRoom(TileMapType& map, int x, int y, int xMaxLength, int yMaxLength, Direction direction)
      {
        // TileMapLayout& staticLayout = map.GetLayout(TileMap::eLayouts::TL_STATIC);

         // Minimum room size of 4x4 tiles (2x2 for walking on, the rest is walls)
         auto xLength = Utils::GetRandomInt(4, xMaxLength, self->_random);
         auto yLength = Utils::GetRandomInt(4, yMaxLength, self->_random);
      ...
      bool DugneonGenerator::MakeFeature(TileMapType& map, int x, int y, int xmod, int ymod, Direction direction)
      {
         //TileMapLayout& staticLayout = map.GetLayout(TileMap::eLayouts::TL_STATIC);
         //TileMapLayout& objectsLayout = map.GetLayout(TileMap::eLayouts::TL_OBJECTS);

         // Choose what to build
         auto chance = Utils::GetRandomInt(0, 100, self->_random);

         if(chance <= self->_chanceRoom)
         {
      ...
      bool DugneonGenerator::MakeFeature(TileMapType& map)
      {
         //TileMapLayout& staticLayout = map.GetLayout(TileMap::eLayouts::TL_STATIC);
         //TileMapLayout& objectsLayout = map.GetLayout(TileMap::eLayouts::TL_OBJECTS);
         ...
         for(; tries != maxTries; ++tries)
         {
            ...
            int x = Utils::GetRandomInt(1, self->_xSize - 2, self->_random);
            int y = Utils::GetRandomInt(1, self->_ySize - 2, self->_random);
            /*
      ...
      bool DugneonGenerator::MakeDungeon(TileMapType& map)
      {
         // TileMapLayout& staticLayout = map.GetLayout(TileMap::eLayouts::TL_STATIC);

         // Make one room in the middle to start things off.
         MakeRoom(map, self->_xSize / 2, self->_ySize / 2, 8, 6, GetRandomDirection(self->_random));

         for(auto features = 1; features != self->_maxFeatures; ++features)
         {
            if(!MakeFeature(map))
            {
      ...
      void WorldGenerator::InitLayouts(TileMapType& map)
      {
         //TileMapLayout& staticLayout = map.GetLayout(TileMap::eLayouts::TL_STATIC);
         //TileMapLayout& objectsLayout = map.GetLayout(TileMap::eLayouts::TL_OBJECTS);
      ...
      bool WorldGenerator::MakeWorld(TileMapType& map)
      {
         const double fx = self->_size / self->_frequency;
         const double fy = self->_size / self->_frequency;
       ...
       //  TileMapLayout& decorLayout = map.GetLayout(TileMap::eLayouts::TL_STATIC_DECOR);
         ...
         for(int x = 0; x < _size; x++) {
            for(int y = 0; y < _size; y++) {
               ...
               // biom dota
               if(val <= 0.32) {
                  eTile waterTile = *select_randomly(waterTileInterval.begin(), waterTileInterval.end(), self->_random);
                  ...
                  eTile flowerTile = *select_randomly(flowerTileInterval.begin(), flowerTileInterval.end(), self->_random);
                  eTile dirtTile = *select_randomly(dirtTileInterval.begin(), dirtTileInterval.end(), self->_random);
                  staticLayout.SetCell(x, y, dirtTile);
                  auto isTree = std::uniform_real_distribution<>(0.0, 1.0)(self->_random);
                  if(isTree < 0.05) decorLayout.SetCell(x, y, flowerTile);
                  if(isTree > 0.8) {
                     eTile treeTile = *select_randomly(treeTileInterval.begin(), treeTileInterval.end(), self->_random);
      ...
      bool WorldGenerator::MakeStaticRoom(TileMapType& map, int x, int y, int xLength, int yLength)
      {
        // TileMapLayout& staticLayout = map.GetLayout(TileMap::eLayouts::TL_STATIC);
